...
public class PepelacGridBuilder : MonoBehaviour
{
    ...
    [Tooltip("Новая конфигурация типов модулей")]
    ...
    private void OnCancelPerformed(InputAction.CallbackContext ctx)
    {
        ...
        {
            Debug.LogWarning("[PepelacGridBuilder] Нельзя снять модуль: не удалось прочитать ModuleCommonData.");
            return;
        }
    ...
    private GameObject SpawnRealModulePrefab(ModuleCommonData data)
    {
        ...
        {
            Debug.LogError($"[PepelacGridBuilder] Не найден эталон '{data.referenceName}' для типа '{data.moduleType}'.");
            return null;
        }
