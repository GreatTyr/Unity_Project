...
public class FuelTankWorkbenchUI : MonoBehaviour, IWorkbenchUI
{
    ...
    private void DrawProgressBar(Rect rect, float progress, string text)
    {
        ...
        if (_progBgTex == null)
            _progBgTex = WorkbenchPopup.MakeTex(1, 1, new Color(0.12f, 0.12f, 0.12f, 1f));
        if (_progFillTex == null)
            _progFillTex = WorkbenchPopup.MakeTex(1, 1, new Color(0.2f, 0.65f, 0.3f, 1f));
