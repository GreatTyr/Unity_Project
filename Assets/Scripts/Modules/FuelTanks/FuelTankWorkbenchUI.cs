...
/// Бак — special-case модуль, но UI приводится к общему шаблону:
/// - параметры верстака
/// - общие параметры
/// - специфичные параметры бака
/// - анализ оболочки
...
public class FuelTankWorkbenchUI : MonoBehaviour, IWorkbenchUI
{
    ...
    private void OnGUI()
    {
        ...
        windowRect = GUI.Window(GetInstanceID(), windowRect, DrawWindow, "Fuel Tank Workbench", _windowStyle);
    ...
    private void DrawComputedSection()
    {
        ...
        DrawGridRow("Высота (Y):", $"<color={dimColor}>{sc.CalcHeight:F3} м</color>", "Полость:", $"{sc.CalcEffectiveVolume:F4} м³");
    ...
    {
        GUILayout.BeginVertical("box");
        GUILayout.Label("Специфичные параметры бака", GetBoldStyle());

        GUILayout.BeginHorizontal();
        ParamBox("Ёмкость", $"{controller.CalcCapacity:F3}");
        GUILayout.EndHorizontal();
