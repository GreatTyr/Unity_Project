...
/// <summary>
/// Полностью автономный контроллер Верстака Генераторов.
...
public class GeneratorWorkbenchController : MonoBehaviour
{
    ...
    public GeneratorDatabase database;
    ...
    {
        GeneratorCalculator.Result result = GeneratorCalculator.Calculate(
            SelectedRef,
            Scaler,
            IsAlloyDecoded,
            AlloyParams
        ...
        CalcSpecificPower = result.specificPower;
        CalcFuelKgPerS = result.fuelKgPerS;
        CalcHeatCapacity = result.heatCapacity;
        CalcMaxTemperature = result.maxTemperature;
        CalcHeatingRate = result.heatingRate;
        CalcEnergyCapacity = result.energyCapacity;

        calcPowerTimesTierPer0001 = result.powerTimesTierPer0001;
        calcFuelPer0001m3Tiered = result.fuelPer0001m3Tiered;
    ...
    {
        string firstLine = GeneratorCode.BuildFirstLine(
            StandardGenerator.TYPE_GENERATOR,
        ...
        string secondLine = GeneratorCode.BuildSecondLine(
            CalcSpecificPower,
            CalcFuelKgPerS,
        ...
        return GeneratorCode.BuildFullCode(firstLine, secondLine, alloyCode);
    }

    public void ApplyBlueprintCode(string code)
    {
        string normalizedCode = GeneratorCode.NormalizeCodeText(code);

        if (string.IsNullOrWhiteSpace(normalizedCode))
        {
            ShowError("Пустой код генератора.");
            return;
        }

        if (!GeneratorCode.TryParseFullCode(normalizedCode, out var parsed))
        {
            ShowError(parsed.ErrorMessage);
            return;
        }

        if (parsed.FirstLine.ModuleType != StandardGenerator.TYPE_GENERATOR)
        {
            ShowError($"Чертёж не относится к модулю типа {StandardGenerator.TYPE_GENERATOR}.");
            return;
        }
        ...
        if (database == null)
        {
            ShowError("База генераторов не назначена.");
            return;
        }

        StandardGenerator foundRef = database.GetByFactionAndBlueprintID(parsed.FirstLine.Faction, parsed.FirstLine.BlueprintId);
        if (foundRef == null)
        {
            ShowError($"Эталон генератора [{parsed.FirstLine.Faction}-{parsed.FirstLine.BlueprintId}] не найден в БД.");
            return;
        }
        ...
        if (refIndex < 0)
        {
            ShowError("Не удалось определить индекс эталона генератора в базе.");
            return;
        }
        ...
        else
        {
            ...
            ShowMessage("Чертёж генератора старого формата: масштаб восстановлен приблизительно.", true);
        }
        ...
        {
            ShowError("Не удалось распознать код сплава генератора.");
            return;
        }

        RecalculateAll();

        var verification = GeneratorVerifier.VerifyFullCodeAgainstCurrent(
            normalizedCode,
            CurrentModuleCode
        );
    ...
    private IEnumerator CraftRoutine()
    {
        ...
        // 1. Списание ресурсов (остается как было)
        ...
        // 2. СОЗДАЕМ ОБЪЕКТ ДАННЫХ НАПРЯМУЮ
        GeneratorData moduleData = new GeneratorData();

        // 3. ЗАПОЛНЯЕМ БАЗУ (через метод, который мы добавили в ModuleCommonData)
        moduleData.SetBaseStats(Scaler, SelectedRef, CurrentModuleCode, alloyCode);

        // 4. ДОЗАПОЛНЯЕМ СПЕЦИФИКУ (то, что раньше делал Initialize)
        ...
        // ... и остальные поля Calc* которые есть в этом контроллере ...
