...
/// <summary>
/// Полный кодек строкового представления генератора.
/// Первая строка поддерживает новый токен sf (scaleFactor):
/// ...-sp78.000-sf1.234567-FACTION-BP
///
...
public static class GeneratorCode
{
    ...
    private const float PowerTolerance = 0.0005f;
    private const float FuelTolerance = 0.00005f;
    ...
    public static string BuildSecondLine(
        ...
        int fuelTier)
    {
        return $"P{Format3(specificPower)}-F{Format4(fuelKgPerS)}-FT{fuelTier}";
    }
    ...
    public static bool TryParseFirstLine(string line, out ParsedFirstLine parsed)
    {
        ...
        {
            parsed.ErrorMessage = "Пустая первая строка кода генератора.";
            return false;
        }
        ...
        if (parts.Length < 7)
        {
            parsed.ErrorMessage = "Неверная первая строка кода генератора.";
            return false;
        }
        ...
        {
            parsed.ErrorMessage = "Не удалось прочитать тир генератора.";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[2], 'm', out parsed.TotalMassKg))
        {
            parsed.ErrorMessage = "Не удалось прочитать массу генератора.";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[3], 'd', out parsed.Durability))
        {
            parsed.ErrorMessage = "Не удалось прочитать прочность генератора.";
            return false;
        }

        if (!TryParseDimensions(parts[4], out parsed.Length, out parsed.Width, out parsed.Height))
        {
            parsed.ErrorMessage = "Не удалось прочитать размеры генератора.";
            return false;
        }

        if (!TryExtractShellPercent(parts, out parsed.ShellPercent))
        {
            parsed.ErrorMessage = "Не удалось прочитать shell percent генератора.";
            return false;
        }
    ...
    public static bool TryParseSecondLine(string line, out ParsedSecondLine parsed)
    {
        ...
        if (string.IsNullOrWhiteSpace(line))
        {
            parsed.ErrorMessage = "Пустая строка параметров генератора.";
            return false;
        }
        ...
        if (parts.Length != 3)
        {
            parsed.ErrorMessage = "Ожидалось 3 секции: P...-F...-FT...";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[0], 'P', out float power))
        {
            parsed.ErrorMessage = "Не удалось прочитать мощность генератора (P...).";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[1], 'F', out float fuel))
        {
            parsed.ErrorMessage = "Не удалось прочитать расход топлива генератора (F...).";
            return false;
        }

        if (!TryParseFuelTier(parts[2], out int fuelTier))
        {
            parsed.ErrorMessage = "Не удалось прочитать тир топлива генератора (FT...).";
            return false;
        }

        parsed.SpecificPower = power;
        parsed.FuelKgPerS = fuel;
        parsed.FuelTier = fuelTier;
        parsed.IsValid = true;
        return true;
    }
    ...
    public static bool TryParseFullCode(string code, out ParsedFullCode parsed)
    {
        ...
        if (lines.Length < 3)
        {
            parsed.ErrorMessage = "Код генератора должен содержать 3 строки.";
            return false;
        }
        ...
        out ParsedFullCode parsed)
    {
        ...
        string secondLine = BuildSecondLine(
            parsed.SecondLine.SpecificPower,
            parsed.SecondLine.FuelKgPerS,
            parsed.SecondLine.FuelTier
        );
    ...
    public static bool AreSecondLinesEquivalent(string a, string b)
    {
        ...
        return NearlyEqual(pa.SpecificPower, pb.SpecificPower, PowerTolerance) &&
               NearlyEqual(pa.FuelKgPerS, pb.FuelKgPerS, FuelTolerance) &&
               pa.FuelTier == pb.FuelTier;
    }
    ...
    private static bool TryParseFuelTier(string source, out int fuelTier)
    {
        fuelTier = 0;

        if (string.IsNullOrEmpty(source) || !source.StartsWith("FT", StringComparison.Ordinal))
            return false;

        return int.TryParse(source.Substring(2), NumberStyles.Integer, CultureInfo.InvariantCulture, out fuelTier);
    }
    ...
    private static string Format4(float value) => value.ToString("F4", CultureInfo.InvariantCulture);
