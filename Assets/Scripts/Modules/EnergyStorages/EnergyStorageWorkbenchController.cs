...
/// <summary>
/// Полностью автономный контроллер Верстака Хранилищ Энергии.
...
public class EnergyStorageWorkbenchController : MonoBehaviour
{
    ...
    private void CalculateCommonOutputs()
    {
        ...
        CalcExplosionRadius = SelectedRef.CalculateExplosionRadius(CalcEnergyCapacity);
    ...
    {
        EnergyStorageCalculator.Result result = EnergyStorageCalculator.Calculate(
            SelectedRef,
            Scaler,
            IsAlloyDecoded,
            AlloyParams
    ...
    {
        string firstLine = EnergyStorageCode.BuildFirstLine(
            StandardEnergyStorage.TYPE_ENERGY_STORAGE,
        ...
        string secondLine = EnergyStorageCode.BuildSecondLine(CalcEnergyCapacity);

        string alloyCode = (IsAlloyDecoded && AlloyCodes.Length > 0 && SelectedAlloyIndex >= 0)
            ? AlloyCodes[SelectedAlloyIndex]
            : "NONE";

        return EnergyStorageCode.BuildFullCode(firstLine, secondLine, alloyCode);
    }

    public void ApplyBlueprintCode(string code)
    {
        string normalizedCode = EnergyStorageCode.NormalizeCodeText(code);

        if (string.IsNullOrWhiteSpace(normalizedCode))
        {
            ShowError("Пустой код батареи.");
            return;
        }

        if (!EnergyStorageCode.TryParseFullCode(normalizedCode, out var parsed))
        {
            ShowError(parsed.ErrorMessage);
            return;
        }

        if (parsed.FirstLine.ModuleType != StandardEnergyStorage.TYPE_ENERGY_STORAGE)
        {
            ShowError($"Чертёж не относится к модулю типа {StandardEnergyStorage.TYPE_ENERGY_STORAGE}.");
            return;
        }
        ...
        if (database == null)
        {
            ShowError("База батарей не назначена.");
            return;
        }

        StandardEnergyStorage foundRef = database.GetByFactionAndBlueprintID(parsed.FirstLine.Faction, parsed.FirstLine.BlueprintId);
        if (foundRef == null)
        {
            ShowError($"Эталон батареи [{parsed.FirstLine.Faction}-{parsed.FirstLine.BlueprintId}] не найден в БД.");
            return;
        }
        ...
        if (refIndex < 0)
        {
            ShowError("Не удалось определить индекс эталона батареи в базе.");
            return;
        }
        ...
        else
        {
            ...
            ShowMessage("Чертёж батареи старого формата: масштаб восстановлен приблизительно.", true);
        }
        ...
        {
            ShowError("Не удалось распознать код сплава батареи.");
            return;
        }

        RecalculateAll();

        var verification = EnergyStorageVerifier.VerifyFullCodeAgainstCurrent(
            normalizedCode,
            CurrentModuleCode
        );
