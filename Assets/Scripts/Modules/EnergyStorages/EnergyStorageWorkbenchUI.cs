...
public class EnergyStorageWorkbenchUI : MonoBehaviour, IWorkbenchUI
{
    ...
    private void Awake()
    {
        controller = GetComponent<EnergyStorageWorkbenchController>();
    }
    ...
    private void DrawComputedSection()
    {
        ...
        {
            GUILayout.Space(10);
            GUILayout.Label($"<color=#FF4444><b>⚠ ГАБАРИТЫ ПРЕВЫШАЮТ КАМЕРУ ВЕРСТАКА</b></color>", GetCenteredBoldStyle());
        }
    ...
    {
        GUILayout.BeginVertical("box");
        GUILayout.Label("Специфичные параметры батареи", GetBoldStyle());

        GUILayout.BeginHorizontal();
        ParamBox("Ёмкость (E)", $"{controller.CalcEnergyCapacity:F3}");
        GUILayout.EndHorizontal();
    ...
    private void DrawCostsAndButtons(bool isCrafting)
    {
        ...
        float alloyAvail = controller.alloyStorage != null &&
    ...
    private int DrawDropdown(string tag, int selected, string[] options)
    {
        ...
        if (GUILayout.Button(options[selected], new GUIStyle(GUI.skin.button) { fontSize = 13 }, GUILayout.MinWidth(150), GUILayout.Height(25)))
        {
