...
public class EnergyStorageWorkbenchUI : MonoBehaviour, IWorkbenchUI
{
    ...
    private void DrawWindow(int id)
    {
        ...
        // Блокируем интерактив, пока идет крафт
        ...
        DrawSeparator();
        DrawSpecificSection(); // ВАЖНО: не DrawGeneratorSpecificSection()
        ...
        // Внизу снова включаем GUI, чтобы рисовать прогресс-бар/кнопки
    ...
    private void DrawCostsAndButtons(bool isCrafting)
    {
        ...
        float metalAvail = controller.resourcesStorage != null
            ? (float)(controller.resourcesStorage.GetGrams((ResourcesStorage.ResourceIndex)(20 + metalTier - 1)) / 1000.0)
            : 0f;
    ...
    {
        GUILayout.BeginHorizontal();
        GUILayout.Label($"<color=#AAAAAA>{l1}</color>", GUILayout.Width(135)); GUILayout.Label(v1, GUILayout.Width(100));
    ...
    private int DrawDropdown(string tag, int sel, string[] opts)
    {
        ...
        if (GUILayout.Button(opts[sel], new GUIStyle(GUI.skin.button) { fontSize = 13 }, GUILayout.MinWidth(150), GUILayout.Height(25)))
            WorkbenchPopup.Show(opts, sel, GUIUtility.GUIToScreenPoint(Event.current.mousePosition), idx => _pendingSelections[tag] = idx);
    ...
    private void InitStyles()
    {
        ...
        if (_windowStyle == null) _windowStyle = new GUIStyle(GUI.skin.window) { normal = { background = _bgTex, textColor = Color.white }, fontSize = 14, fontStyle = FontStyle.Bold };
        if (_panelStyle == null) _panelStyle = new GUIStyle(GUI.skin.box) { normal = { background = _panelTex, textColor = Color.white }, padding = new RectOffset(10, 10, 10, 10), margin = new RectOffset(0, 0, 5, 5) };
        GUI.skin.label.richText = true;
    }
    private GUIStyle GetCenteredBoldStyle() { if (_centeredBold == null) _centeredBold = new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 15 }; return _centeredBold; }
    private GUIStyle GetBoldStyle() { if (_boldStyle == null) _boldStyle = new GUIStyle(GUI.skin.label) { fontStyle = FontStyle.Bold }; return _boldStyle; }
}