...
public static class EnergyStorageCode
{
    ...
    public static string BuildSecondLine(float energyCapacity)
    {
        return $"C{Format3(energyCapacity)}";
    }
    ...
    public static bool TryParseFirstLine(string line, out ParsedFirstLine parsed)
    {
        ...
        {
            parsed.ErrorMessage = "Пустая первая строка кода батареи.";
            return false;
        }
        ...
        if (parts.Length < 7)
        {
            parsed.ErrorMessage = "Неверная первая строка кода батареи.";
            return false;
        }
        ...
        {
            parsed.ErrorMessage = "Не удалось прочитать тир батареи.";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[2], 'm', out parsed.TotalMassKg))
        {
            parsed.ErrorMessage = "Не удалось прочитать массу батареи.";
            return false;
        }

        if (!TryParsePrefixedFloat(parts[3], 'd', out parsed.Durability))
        {
            parsed.ErrorMessage = "Не удалось прочитать прочность батареи.";
            return false;
        }

        if (!TryParseDimensions(parts[4], out parsed.Length, out parsed.Width, out parsed.Height))
        {
            parsed.ErrorMessage = "Не удалось прочитать размеры батареи.";
            return false;
        }

        if (!TryExtractShellPercent(parts, out parsed.ShellPercent))
        {
            parsed.ErrorMessage = "Не удалось прочитать shell percent батареи.";
            return false;
        }
    ...
    public static bool TryParseSecondLine(string line, out ParsedSecondLine parsed)
    {
        ...
        if (string.IsNullOrWhiteSpace(line))
        {
            parsed.ErrorMessage = "Пустая строка параметров батареи.";
            return false;
        }
        ...
        {
            parsed.ErrorMessage = "Не удалось прочитать ёмкость батареи (C...).";
            return false;
        }

        parsed.EnergyCapacity = capacity;
        parsed.IsValid = true;
        return true;
    }
    ...
    public static bool TryParseFullCode(string code, out ParsedFullCode parsed)
    {
        ...
        if (lines.Length < 3)
        {
            parsed.ErrorMessage = "Код батареи должен содержать 3 строки.";
            return false;
        }
        ...
        out ParsedFullCode parsed)
    {
        ...
        string secondLine = BuildSecondLine(parsed.SecondLine.EnergyCapacity);
    ...
    public static bool AreSecondLinesEquivalent(string a, string b)
    {
        ...
        return NearlyEqual(pa.EnergyCapacity, pb.EnergyCapacity, SecondLineTolerance);
    }
